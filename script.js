...
async function initGooglePayButton() {
    ...
    if (!window.paypal || !window.paypal.Googlepay || !window.google || !window.google.payments) {
        return;
    }

    try {
        googlePayConfig = await window.paypal.Googlepay().config();
...
function calcularPrecio() {
    ...
    if (!inputInicio || !inputFin) return 0;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const contacto = document.getElementById("contacto-cliente");

    if (fechaInicio) fechaInicio.addEventListener("change", resetCheckout);
    if (fechaFin) fechaFin.addEventListener("change", resetCheckout);
    if (contacto) contacto.addEventListener("input", resetCheckout);
